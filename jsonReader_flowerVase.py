...
The latest version can be found under jsonReader_flowerVase_v2.py
For traceability of the work process and for future projects, this code will be preserved.
...
for values in datasetDict_flower.values(): #loop through each dataset and their values
    ...
    for file in values["files"]: #loop through all image filenames of current dataset
        ...
        for region in file["regions"]: #loop through all region coordinates in current imagefile
            ...
            x = int(region["x"])
            y= int(region["y"])
            x2 = int(region["x"] + region ["width"]) #dirty solution to get the right image size see jsonReader_flowerVase_v2 for better solution
            y2 = int(region["y"] + region["height"]) #dirty solution to get the right image size see jsonReader_flowerVase_v2 for better solution
